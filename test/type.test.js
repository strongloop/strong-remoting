...
describe('types', function() {
  ...
  describe('Dynamic(val, [ctx])', function() {
    ...
    function _shouldConvert(type) {
      return function(input, expected) {
        var val = new Dynamic(input).to(type);

        var msg = util.format(
          'Expected %j converter to convert %s to %s, got %s instead',
          type, inspect(input), inspect(expected), inspect(val));

        if (expected === 'NaN') return assert(Number.isNaN(val), msg);

        if (Array.isArray(type) || type === 'array') {
          assert.deepEqual(val, expected, msg);
          for (var i = 0; i < val.length; i++) {
            assert.strictEqual(val[i], expected[i], msg);
          }
        } else {
          assert.strictEqual(val, expected, msg);
        }
      };
    }
  });
  ...
  describe('Sloppy HTTP converter', function() {
    ...
    function shouldConvert(input, expected, type, ctx) {
      var val = httpCoerce(input, type || 'any', ctx || {});
      var msg = util.format(
        'Expected %j sloppy converter to convert %s to %s, got %s instead',
        type, inspect(input), inspect(expected), inspect(val));
      // Believe it or not, deepEqual will actually match '1' and 1, so check types
      assert.strictEqual(typeof val, typeof expected, msg);
      assert.deepEqual(val, expected, msg);
    }
