...
RestAdapter.prototype.createHandler = function() {
  ...
  // mount rest route handler
  var customRouter = new RestRouter();
  root.use(customRouter.handle());
  ...
  classes.forEach(function(restClass) {
    ...
    restClass
      ...
      .forEach(function (route) {
        ...
        route.fullPath = restClass.getPath(); //add fullPath for Trie insertion
        customRouter.registerPathAndHandlers(route);
        //Do we need to register above class route?if yes, what's all method verb?
      });
    ...
    methods.forEach(function(m) {
      adapter._registerMethodRouteHandlers(router, m.method, m.route, customRouter);
    });
...
RestAdapter.prototype._registerMethodRouteHandlers = function(router,
  ...
  if(!route.path.includes(':')){
    customRouter.registerPathAndHandlers({verb: verb, fullPath: route.fullPath},
                                          handler);
  }
