...
function coerceAccepts(uarg, desc) {
  ...
  if (targetTypeIsArray && Array.isArray(uarg)) {
    var result = uarg.map(function(arg, ix) {
      // when coercing array items, use only name and type,
      // ignore all other root settings
    ...
    // when the array item type is scalar (not an object), then require
    // all array items to have a valid value
    if (['any', 'object', 'array'].indexOf(targetType[0]) === -1) {
      var doesNotExist = function(it) {
        return it === undefined || it === null;
      };

      if (result.some(doesNotExist)) {
        message = g.f('Invalid value for argument \'%s\' of type ' +
          '\'%s\': %s. Some array items are null or not defined.',
          name, targetType, uarg);
  ...
  if (conversionNeeded) {
    if (Dynamic.canConvert(targetType)) {
      ...
      try {
        ...
        message = g.f(
          'Invalid value for argument \'%s\' of type \'%s\': %s.  Error: %s',
          name, targetType, uarg, message);
        throw badArgumentError(message);
      }
    } else {
      ...
      //console.log('convert result %j', uarg, actualType);
    }
    actualType = SharedMethod.getType(uarg, targetType);
  }
  ...
  if (actualType === 'number' && Number.isNaN(uarg)) {
    throw badArgumentError(g.f('%j must be a {{number}}', name));
  }
  if (actualType === 'integer') {
    if (Number.isNaN(uarg)) {
      throw badArgumentError(g.f('%j must be an {{integer}}', name));
    }
    if (!number.isSafeInteger(uarg)) {
      throw badArgumentError(g.f('%j must be a safe {{integer}}', name));
    }
  }
  return uarg;
}
