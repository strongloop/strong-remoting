...
SharedMethod.prototype.invoke = function(scope, args, remotingOptions, ctx, cb) {
  ...
  if (accepts) {
    for (var i = 0; i < accepts.length; i++) {
      ...
      try {
        uarg = coerceAccepts(uarg, desc, name, ctx);
      } catch (e) {
 ...
 */
function coerceAccepts(uarg, desc, name, ctx) {
  var name = desc.name || desc.arg;
  ...
  var err = converter.validate(ctx, uarg);
  if (err) {
    err.message = g.f('Invalid argument %s. ', name) + err.message;
    throw err;
  }
