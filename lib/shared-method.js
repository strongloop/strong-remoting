...
function coerceAccepts(uarg, desc) {
  ...
  if (conversionNeeded) {
    ...
    try {
      ...
      var message = g.f('invalid value for argument \'%s\' of type ' +
  ...
  if (actualType === 'undefined') {
    if (desc.required) {
      throw new badArgumentError(g.f('%s is a required argument', name));
    } else {
      return undefined;
    }
  }
  ...
  if (actualType === 'number' && Number.isNaN(uarg)) {
    throw new badArgumentError(g.f('%s must be a {{number}}', name));
  }
  if (actualType === 'integer') {
    if (Number.isNaN(uarg)) {
      throw new badArgumentError(g.f('%s must be an {{integer}}', name));
    }
    if (!Number.isSafeInteger(uarg)) {
      throw new badArgumentError(g.f('%s must be a safe {{integer}}', name));
    }
  }
  return uarg;
}
    ...
    default:
      switch (typeof value) {
        ...
        case 'object':
          return value;
        default:
          throw new badArgumentError(g.f('%s must be %s', argName, targetType));
      }
  }
}
