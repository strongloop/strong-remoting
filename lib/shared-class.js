...
SharedClass.prototype.methods = function () {
  ...
  return methods.concat(this._methods);
}
...
SharedClass.prototype.defineMethod = function(name, options) {
  options = options || {};
  var isStatic = !options.prototype;
  var fn;
  if(isStatic) {
    fn = this.ctor[name];
  } else {
    fn = this.ctor.prototype[name];
  }
  this._methods.push(new SharedMethod(fn, name, this, isStatic));
}
