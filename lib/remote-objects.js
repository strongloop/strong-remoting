 ...
 *     var value = new MyType(value);
 *     var error = this.validate(value, typeRegistry, ctx);
 ...
 *     var getData = typeRegistry.getConverter('object').fromSloppyString;
 *     var data = getData(value, typeRegistry, ctx);
 *     return this.fromTypedValue(data);
 *   },
 *
 ...
 * });
 *
 * A deprecated version using a global registry of functions:
 *
 * ```js
...
RemoteObjects.prototype.defineType = function(name, converterOrFn) {
  if (typeof converterOrFn === 'function') {
    deprecated('remoteObjects.defineType(name, fn) is deprecated. ' +
      'Use remoteObjects.defineType(name, converter) instead.');
    return Dynamic.define(name, fn);
  }
  this._typeRegistry.defineType(name, converterOrFn);
};

RemoteObjects.convert =
RemoteObjects.prototype.convert = function(name, fn) {
  deprecated('RemoteObjects.convert(name, fn) is deprecated. ' +
    'Use remoteObjects.defineType(name, converter) instead.');
