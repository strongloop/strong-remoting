...
HttpContext.prototype.getArgByName = function(name, options) {
  var req = this.req;
  var args = (req.params && req.params.args) ||
    (req.body && req.body.args) ||
    (req.query && req.query.args);
  ...
  */
  ...
  // req.query
  // req.header
  var objectsWithArg = [args, req.params, req.body, req.query].filter(function(obj) {
    return (obj && obj[name] !== undefined);
  });
  // Use the first object we might have found the arg in, try headers last.
  var arg = objectsWithArg.length ? objectsWithArg[0][name] : req.get(name);
